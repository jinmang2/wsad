...
HEADS = {
    ...
    "pel4vad": dict(runner="pel4vad", text=False, magnitude=False, segment=32, length=None, steps=6290, batch=64, lr=5e-4, wd=0.0, crop_sampling="random"),  # [ref] PEL4VAD: 50 epochs x (16100 crop-samples / bs 128) = 6290 steps, Adam lr=5e-4 (paper 0.8676 on 1024-d I3D)
}
    ...
    return OmegaConf.create(
        {
            ...
            # heads whose official recipe treats each crop as its own training sample
            "train_crop_sampling": spec.get("crop_sampling"),
            "ground_truth": None,
